...
先运行只读 O6 连接检查，该命令不会发送位置动作：

```powershell
python -m human_mimic_demo.scripts.check_o6_connection `
  --config human_mimic_demo\configs\demo_windows.json
```

确认返回六维 state 后，才允许以 hand-only 模式接入视觉。该模式不会连接 UR5：

先执行逐轴小幅运动测试，运行后还需要手动输入 `MOVE O6`：

```powershell
python -m human_mimic_demo.scripts.test_o6_motion `
  --config human_mimic_demo\configs\demo_windows.json `
  --step 20 `
  --i-understand-hardware-risk
```

确认六个轴方向正确并自动回到全张开后，再运行视觉 hand-only：
