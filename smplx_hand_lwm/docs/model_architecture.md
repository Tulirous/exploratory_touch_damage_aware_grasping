...
`HMWM-AdaLN-Cross-v2` 回到原 HMWM 的 query/memory 拓扑：learned future
queries 在每层通过 cross-attention 读取完整 context memory，memory 只作为
K/V 且不被 LA 调制。与原 HMWM 的区别仅是删除输入处一次 LA 加法，并用每层
六路 AdaLN-Zero 调制 future-query self-attention 与 FFN；cross-attention
保持普通 pre-norm residual，以确保训练初始化时 context 路径已经可用。
