...
from latent_action_idm.models import InverseDynamicsTransformer, LatentWorldModelDecoder, Stage1LaWAM
...
def main() -> None:
    ...
    idm = InverseDynamicsTransformer(
    ...
    posterior = idm(visual_t, visual_future, state_t, sample=False)
    print("IDM latent_mu:", tuple(posterior["latent_mu"].shape))

    lawm = LatentWorldModelDecoder(
        visual_token_dim=args.token_dim,
        latent_action_dim=args.latent_dim,
        hidden_dim=args.hidden_dim,
        num_layers=args.layers,
    ...
    pred_future = lawm(visual_t, posterior["latent_mu"])
    print("LaWM predicted future:", tuple(pred_future.shape))
    ...
    print("Stage1 latent_mu:", tuple(outputs["latent_mu"].shape))
    print("Stage1 predicted_visual_future:", tuple(outputs["predicted_visual_future"].shape))
    print("Stage1 predicted_state_future:", tuple(outputs["predicted_state_future"].shape))
